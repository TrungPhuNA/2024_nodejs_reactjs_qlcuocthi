...
const MultiSelect: React.FC<DropdownProps> = ({ id, ...props }: any) => {
	...
	const setValueForm = (newOptions: any = []) => {
		let formSelect = newOptions.reduce((newItem: any, item: any) => {
			if(item?.selected) {
				newItem.push(item.value);
			}
			return newItem;
		}, []);
		console.log("form------->", formSelect);
		setField(formSelect, props.obj_key, props.form, props.setForm)
	}
		...
		<div className="relative z-50">
			...
			<div>
				<select className="hidden" id={id}>
					{props.data?.length > 0 && props?.data?.map((item: any, key: any) => {
						return (
