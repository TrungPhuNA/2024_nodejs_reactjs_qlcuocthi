...
const ClassPage: React.FC = () => {
	...
	const getDataList = async (filters: any) => {
		...
		const response: any = await CLASS_SERVICE.getList(filters);
	...
	return (
		<DefaultLayout>
			...
			<FormCreateOrUpdateClass open={open}
							...
							<tbody>
								{dataList.map((packageItem:any, key: any) => (
									<tr key={key}>
										...
										<td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
											<p className="text-black dark:text-white">
												{packageItem.unit == 'a' ? "A" : (packageItem.unit == 'b' ? "B" : "C")}
											</p>
										</td>
										<td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
											<p className="text-black dark:text-white">
												{packageItem?.school?.name}
											</p>
										</td>
										<td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
											<p
										...
										<td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
											<div className="flex items-center space-x-3.5">
												...
												<button className="hover:text-primary" onClick={() => updateData(packageItem)}>
													<svg
														className="fill-current"
														width="18"
														height="18"
